...
useEffect(() => {
    ...
    const payload = {
        ...
        try {
            if (editData) {
                const res = await axios.put(`http://localhost:5000/api/transactions/${editData._id}`, editData);
        ...
        // Don't need finally fetchTransactions here as state update triggers re-render
    };
    ...
    <div className="h-full overflow-y-auto scroll-smooth mt-11 pb-20 bg-transparent from-green-200 via-emerald-100 to-lime-200 dark:bg-transparent dark:from-gray-800 dark:via-gray-900 dark:to-gray-800">
        <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {/* Total Income Card */}
                <motion.div 
                    className="p-6 rounded-2xl bg-white/90 backdrop-blur-md shadow-lg border border-white/40 dark:border-0 flex flex-col items-center dark:bg-gray-700/60 dark:border-gray-650/60" 
                ...
                >
                    ...
                    <p className="text-red-500 font-bold text-2xl">₹{totalExpense.toFixed(2)}</p>
                </motion.div>
                {/* Net Balance Card */}
                <motion.div 
                    className="p-6 rounded-2xl bg-white/90 backdrop-blur-md shadow-lg border border-white/40  dark:border-0 flex flex-col items-center dark:bg-gray-700/60 dark:border-gray-650/60" 
            ...
            >
                ...
                <div className="flex gap-2">
                    <button type="submit" className={`px-4 py-2 rounded font-semibold ${editData ? "bg-blue-600 hover:bg-blue-700" : "bg-green-600 hover:bg-green-700"} text-white transition`}>
                        ...
                        {filteredTransactions.map(tx => (
                            <motion.div
                                ...
                                className={`p-4 rounded-2xl shadow-lg border 
                                    ${tx.type === "income" 
                                        ? "bg-green-100 border-green-300 dark:bg-green-700/50 dark:border-green-800" 
